...
###############################################################################
if [[ $# -lt 1 ]]; then
  echo "Uso: $0 YYYYMMDD [TABLA … routines]" >&2
  exit 1
fi

DATE="$1"; shift           # quita la fecha
RESTORE_ROUTINES=false
TABLE_LIST=("$@")          # todos los demás args tal cual

# eliminar 'routines' de la lista y setear flag
for i in "${!TABLE_LIST[@]}"; do
  if [[ ${TABLE_LIST[$i]} == routines ]]; then
